...
5. **Grouping the buttons: the `groupByLocation` prop**:
   ...
   *  **`groupByLocation` set to `false`** — for features specific to the club *and* the league's season (League + Season + Club Name + Location), like showing the club's teams registration status and kudos standings. One button per managed club is visualised, labelled `"<club_location> / <league>"`. **`MyClubTeams.tsx` is the first page to use this branch**; `MyClubStandings.tsx` uses it too.
   ...
   *  Note the `false` label carries the league but **not the season**, so a manager of the same club in two seasons of one league would see two buttons with identical labels. The keys differ; only the labels collide. **Reviewed when My Club Teams was implemented, and deliberately NOT fixed:** the colliding state is only reachable through a *config error*, because only one season per league can be active at a time. Making the label more specific would hide that error rather than surface it. Instead `ManagedClubsCard` detects duplicate labels and emits `console.error` in the shape of `AuthContextParsers.ts` — see the comment on the effect, which says so, so that a future reader does not "fix" the label.
...
* **The matching + time-window logic lives in ONE place**: `selectActiveManagedClubs(...)` in [clubUtils.ts](TTLeaguePlayersApp.FrontEnd/src/utils/clubUtils.ts). All three club-manager pages call it. It returns `{ active, excluded }` — each `active` entry carrying the **matched `ActiveSeasonDataSource`**, which the promotion pages used to compute and discard and which `MyClubTeams` needs to build its processor; each `excluded` entry carrying a reason (`CONFIG_MISSING` / `DATA_SOURCE_NOT_FOUND` / `SEASON_NOT_ACTIVE` / `UNEXPECTED_ERROR`). It is a plain function, not a hook, because it does no I/O: its inputs are the Cognito clubs already in memory, the build-time config, and the clock.
* **A club that cannot be resolved is excluded and logged, never thrown** — one broken config entry must not cost a manager the clubs that are fine.
* **Club Processor Factory Pattern**: The club-manager pages construct their processing logic via `createManagedClubProcessor(...)` in [ManagedClubProcessorFactory.ts](TTLeaguePlayersApp.FrontEnd/src/service/active-season-processors/ManagedClubProcessorFactory.ts). This maps the config strategy key (`custom_club_processor`) to the corresponding parsing engine class, and injects the club name, the club location and the scraping parameters.
	* **`avoidCORS` must be passed `true` by every club-manager page.** It defaults to `false`, and `false` sends the club-page scrape straight into a CORS failure that renders the list blank with nothing on screen to explain it. The parameter is last and defaulted, so omitting it fails silently at run time rather than at compile time.
	* This is a **separate port** from the `ActiveSeasonProcessor` used by the player/captain flows, and deliberately so. `ActiveSeasonProcessor.getTeamFixtures()` and `getTeamPlayers()` need a division and a team, which come from `custom:active_seasons`; a club manager has only a club, from `custom:managed_clubs`. Different binding tuple, different interface. Both are nonetheless resolved against the **same** `active_seasons_data_source` entry, matched on league + season.
	* The factory wraps the processor in `ManagedClubProcessorWithLocalStorageCache` (same 72h fresh / 6d stale SWR policy as the fixtures cache), under the key `cache_club_{league}_{season}_{club_location}_{club}` — distinct from the active-season keys `cache_{league}_{season}_{division}_{team}_{fixtures|players}`, since a club's teams do not vary by division or team. **The location is part of the key because a club is identified by location AND name**: two towns can host clubs of the same name, and the registrations cache key already carried the location.
...
* [ManagedClubsCard.tsx](TTLeaguePlayersApp.FrontEnd/src/components/ui/ManagedClubsCard.tsx), used by [PromoteMyClub.tsx](TTLeaguePlayersApp.FrontEnd/src/pages/PromoteMyClub.tsx) and [PromoteMyTournaments.tsx](TTLeaguePlayersApp.FrontEnd/src/pages/PromoteMyTournaments.tsx) (both `groupByLocation`), and by [MyClubTeams.tsx](TTLeaguePlayersApp.FrontEnd/src/pages/MyClubTeams.tsx) (`groupByLocation={false}`)
...
2. **Fetch that page** (`https://www.tabletennis365.com/CentralLondon/Clubs/<slug>`, through the CORS proxy in the browser) and parse its Teams table, `table.tt-contact-table`, whose columns are League | Division | Team | Captain: the Team cell is the team name and the Division cell, spelled as the app spells it (`Premier`, `Division 4`), is the team's division.
...
* **Assumption: the config and the club's page on the league site are always current for the current season.** Nothing reconciles them. A team added to the club page is invisible to the app until the registrations cache expires, and a club renamed on the league site stops resolving until `club_teams` is edited by hand.
* **No season filtering is applied.** The club page's League column shows a season label such as `"Winter 2026-27"`, which does not textually match the config's `season` (`"2025-2026"`), and the page carries no season in its URL (a `?leagueName=` parameter is ignored by the site). Every listed team is returned — and since the site's re-platforming (September 2026) the page lists the season the site considers current, which is the *next* season once it is published: while `active_seasons_data_source` still points at 2025-2026, the club pages return the 2026-27 teams and divisions. Accepted knowingly on 2026-09-21.
