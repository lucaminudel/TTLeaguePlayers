...
import { RATE_INFO_MODAL_GUID, DISPUTES_INFO_TITLE, DisputesInfoBody } from '../common/infoModalMessages';
...
export const ActiveSeasonCard: React.FC<ActiveSeasonCardProps> = ({ season, processor, isExpanded, onToggle }) => {
    ...
    // Handler for the info modal's OK button: carry on. Persisting the opt-out is InfoModal's job.
    ...
    return (
        <div className="bg-primary border border-gray-600 rounded-lg p-0" data-testid="active-season-card">
            ...
            {rateInfoModalFixture && (
                <InfoModal
                    guid={RATE_INFO_MODAL_GUID}
                    title={DISPUTES_INFO_TITLE}
                    body={<DisputesInfoBody />}
                    testId="rate-info-modal"
                    onOk={handleRateInfoModalOk}
                />
            )}
        </div>
    );
