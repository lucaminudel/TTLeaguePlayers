                    ...
                    <ul className="flex flex-col space-y-6 text-center">
                        {!isAuthenticated ? (
                            ...
                            <li className="text-center">
                                <div className="text-2xl text-red-600 mb-8" data-testid="main-menu-user-info">
                                    <div data-testid="main-menu-welcome-message">
                                        Welcome, {welcomeName}
                                    </div>
                                    {firstSeason && (
                                        <div className="text-xl mt-4" data-testid="main-menu-first-season">
                                    ...
                                    {activeSeasons.slice(1).map((season, index) => (
                                        <div key={`${season.league}-${season.season}-${season.team_name}-${season.person_name}-${String(index)}`} className="text-xl mt-2" data-testid="main-menu-additional-season">
                        ...
                        {menuItems.map((item) => (
                            <li key={item}>
                                <a
                                    ...
                                    className="text-2xl font-bold text-main-text hover:text-action-accent transition-colors block py-2"
                                    onClick={toggleMenu}
                                    data-testid={`main-menu-nav-${item.toLowerCase().replace(/\s+/g, '-')}`}
                                >
                                    {item}
                                </a>
                            </li>
